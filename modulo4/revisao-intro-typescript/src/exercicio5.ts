...
function checaRenovacaoRG(anoAtual: number, anoNascimento: number, anoEmissao: number): any {
    let idade = anoAtual - anoNascimento
    let tempoCarteira = anoAtual - anoEmissao
   
     if(idade <= 20 ) {
         return tempoCarteira >= 5 ? "passou dos 5 anos precisa renovar" : "Ainda não passou os 5 anos, portanto, não precisa renovar"
       
      }else if(idade >= 20 || idade <= 50) {
         return tempoCarteira >= 10 ? "passou dos 10 anos precisa renovar" : "Ainda não passou os 10 anos, portanto, não precisa renovar"
       
      }else if(idade > 50) {
         return tempoCarteira >=15 ? "passou dos 15 anos precisa renovar" : "Ainda não passou os 15 anos, portanto, não precisa renovar"
   ...
   const verificandoRenovacao: number = checaRenovacaoRG(2022, 1998, 2017);

   console.log(verificandoRenovacao);
